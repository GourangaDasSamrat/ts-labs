...
class TaskRenderer {
    ...
    complete(title, del) {
        const f = document.createDocumentFragment();
        const li = document.createElement("li");
        li.innerHTML = `<span>${this.esc(title)}</span><button type="button">Delete</button>`;
...
class TaskManager {
    inc = storageManager.load("incompleteTasks");
    comp = storageManager.load("completeTasks");
    r = new TaskRenderer();
    add(t) {
        if (!t.trim())
            return;
    ...
    render() {
        const i = dom.get("#items");
        const c = dom.get(".complete-list ul");
        i.innerHTML = "";
        c.innerHTML = "";
        const ifrag = document.createDocumentFragment();
        const cfrag = document.createDocumentFragment();
        this.inc.forEach((t) => ifrag.appendChild(this.r.incomplete(t, (x) => this.complete(x))));
        this.comp.forEach((t) => cfrag.appendChild(this.r.complete(t, (x) => this.delete(x))));
        i.appendChild(ifrag);
        c.appendChild(cfrag);
    }
}
